...
namespace TsAudio.Decoders.Mp3
{
    ...
    abstract class LayerIIDecoderBase : LayerDecoderBase
    {
        ...
        int DecodeSamples(Span<float> ch0, Span<float> ch1)
        {
            ...
            for (int ch = startChannel; ch <= endChannel; ch++)
            {
                ...
                for (int gr = 0; gr < _granuleCount; gr++)
                {
                    for (int ss = 0; ss < SSLIMIT; ss++)
                    {
                        ...
                        var channelBuf = ch == 0 ? leftBuf : rightBuf;
                        _polyPhaseBuf.AsSpan(0, SBLIMIT).CopyTo(channelBuf.Slice(idx - SBLIMIT, SBLIMIT));
