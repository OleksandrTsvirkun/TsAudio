...
namespace TsAudio.Decoders.Mp3
{
    internal class Huffman
    {
        ...
        internal static void Decode(BitReservoir bbitReserviorr, int table, out float x, out float y)
        {
            ...
            {
                var val = DecodeSymbol(bbitReserviorr, table);
                ...
                x = GetBits(bbitReserviorr, ix, linBits);
                y = GetBits(bbitReserviorr, iy, linBits);
            }
        }

        internal static void Decode(BitReservoir bitReservior, int table, out float x, out float y, out float v, out float w)
        {
            var val = DecodeSymbol(bitReservior, table);

            v = w = x = y = 0;

            InverseBit(bitReservior, ref v, val, 0x8);
            InverseBit(bitReservior, ref w, val, 0x4);
            InverseBit(bitReservior, ref x, val, 0x2);
            InverseBit(bitReservior, ref y, val, 0x1);
        }

        // In C#, bit operations are faster than the tree traversal the spec is written for.
        private static byte DecodeSymbol(BitReservoir bitReservior, int table)
        {
            ...
            int readBits;
            int bits = bitReservior.TryPeekBits(maxBits, out readBits);
            if (readBits < maxBits)
            {
            ...
            while (node != null && node.Length <= readBits)
            {
                if ((bits & node.Mask) == node.Bits)
                {
                    ...
                    bitReservior.SkipBits(node.Length);
                    break;
                }
                node = node.Next;
            }
        ...
        private static float GetBits(BitReservoir bitReservior, int ix, int linBits)
        {
            float x;
            if(linBits > 0 && ix == 15)
            {
                ix += bitReservior.GetBits(linBits);
            }

            if(ix != 0 && bitReservior.Get1Bit() != 0)
            {
                x = -_floatLookup[ix];
            }
            else
        ...
        private static void InverseBit(BitReservoir bitReservior, ref float v, byte val, int comparer)
        {
            if((val & comparer) != 0)
            {
                if(bitReservior.Get1Bit() == 1)
                {
                    v = -_floatLookup[1];
                }
                else
